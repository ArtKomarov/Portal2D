...
bool PhysElem::getBlock(int side) const {
    switch (side) {
    case LEFT:
        return block_.left_;
        break;
...
sf::Vector2f PhysElem::getSpeed() const {
    return sf::Vector2f(speedX_, speedY_);
}

sf::Vector2f PhysElem::getAccel() const {
    return sf::Vector2f(accelX_, accelY_);
}
...
void pushOff(PhysElem &this_elem, GrElem &elem, sf::Vector2u windowSize) {
    ...
    pushToSide(this_elem, elem, side);
...
//    std::cout << "LDist = " << LDist << " RDist = " << RDist << " TDist = " << TDist << " BDist = " << BDist << std::endl;
    ...
    for(int i = 0; i < 4; i++) {
        if(TDist <= LDist && TDist <= RDist && TDist <= BDist) {
            if(elem.getPosition().y - this_elem.getGlobalBounds().height >= 0) {
        ...
        if(RDist <= LDist && RDist <= TDist && RDist <= BDist) {
            if(elem.getPosition().x + elem.getGlobalBounds().width + this_elem.getGlobalBounds().width <= windowSize.x) {
        ...
        if(LDist <= RDist && LDist <= TDist && LDist <= BDist) {
            if(elem.getPosition().x - this_elem.getGlobalBounds().width >= 0) {
        ...
        if(BDist <= LDist && BDist <= RDist && BDist <= TDist) {
            if(elem.getPosition().y + elem.getGlobalBounds().height + this_elem.getGlobalBounds().height <= windowSize.y) {
                ...
                BDist = std::numeric_limits<float>::max(); // for future comparsion
        }
    }
...
sf::Vector2i PhysElem::Move() {
    ...
    std::cout << __PRETTY_FUNCTION__ << ": dx=" << speedX_ * elapsedTimeX << ", dy=" << speedY_ * elapsedTimeY << std::endl;
